...
async function createFacePair(sock, requesterJid) {
  ...
  if (faceSockets.has(number)) {
    await send(sock, requesterJid, {
      text: "برای شما یک اتصال در حال ساخت است. QR قبلی را اسکن کن."
    });
    return;
  }
  ...
  try {
    ...
    if (state.creds.registered) {
      await send(sock, requesterJid, {
        text: "حساب شما قبلاً به ربات وصل شده است."
      });
      return;
    }
      ...
      try {
        ...
        if (connection === "open") {
          await send(sock, requesterJid, {
            text:
              "اتصال موفق شد.\n" +
              "حساب شما به‌صورت جداگانه به ربات وصل شد."
          });
        }
        ...
        console.log("Face Connection Error:", e.message);
      }
    });
    ...
    face.ev.on("messages.upsert", async event => {
      try {
        ...
        console.log("Face Message Error:", e.message);
      }
    });

    await send(sock, requesterJid, {
      text:
        "در حال ساخت QR اختصاصی شما...\n" +
        "چند لحظه صبر کن."
    });
...
async function startBot() {
  ...
  sock.ev.on("connection.update", async update => {
    ...
    if (qr) { console.log("QR دریافت شد؛ برای .pair آماده است."); }
