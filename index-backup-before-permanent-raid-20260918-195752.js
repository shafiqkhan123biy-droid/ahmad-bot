...
// ===== TIKTOK_DOWNLOADER_SYSTEM_END =====
      ...
      if (
        ...
        if (!senderIsAdmin) {
          await send(sock, jid, {
            text: "❌ فقط مدیر گروپ می‌تواند RAID را تغییر دهد."
          });
          return;
        }

        settings.raid = lower.endsWith("on");
        saveSettings();
        ...
        await send(sock, jid, {
          text: settings.raid
            ? "╭━━━〔 🚨 RAID MODE 〕━━━╮\n" +
              "┃\n" +
              "┃ 🛡️ حالت ضدحمله فعال شد\n" +
              "┃ ⚡ محافظت ویژه: فعال\n" +
              "┃ 👻 ربات آماده مقابله با اسپم است.\n" +
              "┃\n" +
              "╰━━━━━━━━━━━━━━━━━━╯"
            : "╭━━━〔 🛡️ RAID MODE 〕━━━╮\n" +
              "┃\n" +
              "┃ 🔴 حالت ضدحمله خاموش شد\n" +
              "┃ ⚡ محافظت ویژه: غیرفعال\n" +
              "┃\n" +
              "╰━━━━━━━━━━━━━━━━━━╯"
        });
        ...
        try {
          ...
          await send(sock, jid, {
            text:
              ...
              "┃ 📅 آخرین فعالیت: " +
                (profileStats?.lastMessage || "نامشخص") + "\n" +
              "┃\n" +
              "╰━━━━━━━━━━━━━━━━━━━━╯"
          });
        } catch (e) {
      ...
      if (
        ...
        if (!senderIsAdmin) {
          ...
          if (botIsAdmin) {
            ...
            await send(sock, jid, {
              text:
                "لینک حذف شد.\nبرای حذف فرستنده، اح‍ـــمـــدبــݪاݪ باید مدیر گروه باشد."
            });
          }
...
sock.ev.on("call", calls => {
  try {
    ...
    console.log("CALL DEBUG ERROR:", e.message);
  }
});
