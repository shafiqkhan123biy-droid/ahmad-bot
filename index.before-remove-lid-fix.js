    ...
    try {
      ...
      if (removeCommand) {
        ...
        if (!targets.length) {
          await send(sock, jid, {
            text:
              "برای حذف عضو، روی پیام او ریپلای کن و .remove بزن.\n\n" +
              "یا عضو را منشن کن و بنویس:\n" +
              ".remove"
          });
          return;
        }
        ...
        for (const target of targets) {
          ...
          try {
            ...
            await send(sock, jid, {
              text:
                "حذف عضو انجام نشد.\n" +
                "ممکن است این عضو با LID شناسایی شده باشد یا واتساپ اجازه حذف نداده باشد."
            });
          }
        }
